---
title: 'JavaScript | 什麼是提升 ( hoisting )？'
description: 'JavaScript (JS) 在運行時會分成兩個階段，1.「創建階段」和 2.「執行階段」。在創建階段，JS 會為所有變數和函式分配記憶體空間，就好像把變數和函式「宣告」**提升**到最前面，並不是真的會移動程式碼，接下來，執行階段才進行賦值。'
---
