---
title: 'TypeScript | 什麼是 TypeScript'
description: 'TypeScript (TS) 是 JavaScript 的一個超集，在 JavaScript (JS) 的基礎上提供了**型別系統**和對 ES6 的支援。TS 會編譯成純 JS，確保與任何運行 JS 的環境（包括瀏覽器、伺服器和任何作業系統）兼容。TS 有靜態型別檢查器，在編譯階段 (執行前) 就先抓出可能的錯誤或型別錯誤，如下範例。'
---
